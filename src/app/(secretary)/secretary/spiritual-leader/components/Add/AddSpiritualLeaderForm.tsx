...
import AddSpiritualLeaderButton from "./AddSpiritualLeaderButton";
import { addSpiritualLeader } from "@/app/(secretary)/secretary/spiritual-leader/_actions/addSpiritualLeader";
import { useRouter } from "next/navigation";
import Link from "next/link";

export default function AddSpiritualLeaderForm({centers} : {centers: any}) {
        ...
        <div className="flex flex-col items-center justify-center m-auto h-screen align-middle">
          <form
            ...
            action={async (formDta: FormData) => {
              const result = await addSpiritualLeader(
                formDta
              ).then((result) => {
                if (result === null) {
                  throw new Error("Spiritual leader is null");
                }
                return result;
              });
              if (result?.error) {
